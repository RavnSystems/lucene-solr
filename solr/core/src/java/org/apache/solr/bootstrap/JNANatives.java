...
class JNANatives {
    ...
    static void tryVirtualLock() {
        final List<Long> notLockableAddresses = new ArrayList<>();
        final List<Long> failedAddresses = new ArrayList<>();
        logger.debug("Trying virtual lock");
        ...
        try {
            ...
            final ByteSizeValue heapInit = JvmInfo.jvmInfo().getMem().getHeapInit();
            JNAKernel32Library.SizeT size = new JNAKernel32Library.SizeT(heapInit.getBytes() + (1024 * 1024));
            if (!kernel.SetProcessWorkingSetSize(process, size, size)) {
                ...
                logger.info("Trying to lock memory. Heap size is {} MB", heapInit.getMb());
                ...
                while (kernel.VirtualQueryEx(process, new Pointer(address), memInfo, memInfo.size()) != 0) {
                    iterations++;
                    final boolean isCommited = memInfo.State.longValue() == JNAKernel32Library.MEM_COMMIT;
                    final boolean pageIsUnprotected = (memInfo.Protect.longValue() & JNAKernel32Library.PAGE_NOACCESS) != JNAKernel32Library.PAGE_NOACCESS
                            && (memInfo.Protect.longValue() & JNAKernel32Library.PAGE_GUARD) != JNAKernel32Library.PAGE_GUARD;
                    boolean lockable = isCommited && pageIsUnprotected;
                    if (lockable) {
                        logger.debug("Memory is lockable, trying to lock it");
                        if(kernel.VirtualLock(memInfo.BaseAddress, new JNAKernel32Library.SizeT(memInfo.RegionSize.longValue()))){
                            logger.debug("Memory locked");
                        }
                        else{
                            logger.debug("Unable to lock :(");
                            failedAddresses.add(address);
                        }
                    }
                    else{
                        logger.debug("Adress not lockable: {}", pageIsUnprotected? "Memory not commited": "Page is protected");
                        notLockableAddresses.add(address);
                    }
                    // Move to the next region
                    address += memInfo.RegionSize.longValue();
                }
                LOCAL_MLOCKALL = true;
                logger.info("Amount of failed addresses: {} / {}", failedAddresses.size(), iterations);
                logger.info("Amount of not lockable addresses: {} / {}", notLockableAddresses.size(), iterations);
            }
        } catch (UnsatisfiedLinkError e) {
            // this will have already been logged by Kernel32Library, no need to repeat it
        } finally {
