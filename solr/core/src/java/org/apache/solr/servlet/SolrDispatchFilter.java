...
public class SolrDispatchFilter extends BaseSolrFilter {
  ...
  private void setupCpuAffinity(){
    final String cpusProperty = System.getProperty("cpuAffinity");
    if(cpusProperty!=null && Constants.WINDOWS){
      log.info("Setting cpu affinity");
      final int[] cpus = Arrays.stream(cpusProperty.split(",")).mapToInt(Integer::parseInt).toArray();
      Natives.cpuAffinity(cpus);
    }
  }
